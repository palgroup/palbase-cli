...
func TestEmitTypeScriptRegistration(t *testing.T) {
	...
	must(t, out, "// AUTO-GENERATED by `palbase web gen` — DO NOT EDIT.")
	must(t, out, "// Regenerate: palbase web gen (or automatically via the predev/prebuild script)")

	// --- Imports ---
	must(t, out, "import type { BackendError, CallOptions } from '@palbase/web';")
	must(t, out, "import { __configure, __registerNamespaces } from '@palbase/web/internal';")
...
// codes are skipped consistently: no class, no errors-map entry, no orphan
// Data interface — with the loud comment at the descriptor site.
func TestEmitTypeScriptTypedErrors_ReservedCode(t *testing.T) {
	ops := []swiftOp{
		{
			...
			errors: []swiftErrorDef{
				{name: "unauthorizedAccess", code: "unauthorized", status: 401, description: "Not authorized",
					data: &swiftSchema{kind: "object", props: []swiftProp{{name: "why", schema: swiftSchema{kind: "string"}, required: true}}}},
	...
	// reserved code skipped with comment at the descriptor site
	...
	// reserved-code error with data must NOT emit an orphan Data interface
	if strings.Contains(out, "UsersLoginUnauthorizedAccessData") || strings.Contains(out, "why:") {
		t.Errorf("reserved infra code must not emit an orphan Data interface:\n%s", out)
	}
...
func TestEmitTypeScriptTypedErrors_NoErrors(t *testing.T) {
	...
	must(t, out, "import type { CallOptions } from '@palbase/web';")
...
// TestEmitTypeScriptTypedErrors_ZeroOps locks the zero-op output shape:
// header + @palbase/web/internal import + configure + single-line empty registration.
// NO `import type ... from '@palbase/web'` line (neither BackendError nor CallOptions
// is referenced — emitting them is TS6196 under noUnusedLocals) and NO empty
...
func TestEmitTypeScriptTypedErrors_ZeroOps(t *testing.T) {
	...
	if strings.Contains(out, "import type") {
		t.Errorf("zero ops → no type-only import from '@palbase/web':\n%s", out)
	}
	if strings.Contains(out, "BackendError") || strings.Contains(out, "CallOptions") {
		t.Errorf("zero ops → neither BackendError nor CallOptions may appear:\n%s", out)
	}
	if strings.Contains(out, "declare module") {
		t.Errorf("zero ops → empty declare module block must be omitted:\n%s", out)
	}
	if strings.Contains(out, "// ── Typed errors ──") {
...
// TestEmitTypeScriptImports_ErrorOpDroppedByGate locks that `BackendError` is
// NOT imported when every error-bearing op is dropped by a per-op gate — the
// import must be derived from what is ACTUALLY emitted, not a pre-scan
// (stray import = TS6196 for consumers compiling with noUnusedLocals).
func TestEmitTypeScriptImports_ErrorOpDroppedByGate(t *testing.T) {
	boom := []swiftErrorDef{{name: "boom", code: "boom_code", status: 400}}

	t.Run("nested-reserved gate", func(t *testing.T) {
		ops := []swiftOp{
			{operationID: "todos.then", method: "POST", path: "/todos/then", errors: boom},
			{operationID: "todos.create", method: "POST", path: "/todos/create"},
		}
		out := emitTypeScript(ops, tsGeneratedConfig{URL: "https://x.example.com", APIKey: "k", Branch: "main"})
		...
		must(t, out, `// codegen: skipped operation "todos.then" (reserved segment "then")`)
		if strings.Contains(out, "BackendError") {
			t.Errorf("error-bearing op dropped by nested-reserved gate → no BackendError import:\n%s", out)
		}
		if strings.Contains(out, "// ── Typed errors ──") {
			t.Errorf("no surviving errors → Typed errors section must be absent:\n%s", out)
		}
	})

	t.Run("type-name collision gate", func(t *testing.T) {
		ops := []swiftOp{
			{operationID: "widgets.get", method: "GET", path: "/widgets/get"},
			{operationID: "widgetsGet", method: "POST", path: "/widgets-get", errors: boom},
		}
		out := emitTypeScript(ops, tsGeneratedConfig{URL: "https://x.example.com", APIKey: "k", Branch: "main"})
		...
		must(t, out, `// codegen: skipped type "WidgetsGet" (collides with operation "widgets.get")`)
		if strings.Contains(out, "BackendError") {
			t.Errorf("error-bearing op dropped by collision gate → no BackendError import:\n%s", out)
		}
		if strings.Contains(out, "// ── Typed errors ──") {
			t.Errorf("no surviving errors → Typed errors section must be absent:\n%s", out)
		}
...
// TestEmitTypeScriptTypedErrors_ClassNameDedup locks the class-NAME seen-set:
// camel + snake error names (roomLocked / room_locked) both PascalCase to the
// same class name → TS2300 duplicate identifier without the dedup. First-wins
// (stable sort by code) + loud comment; the dropped error must not lift into
// the survivor's class via the errors map, and its Data interface must not
// emit as an orphan (it would also be a duplicate-name interface).
func TestEmitTypeScriptTypedErrors_ClassNameDedup(t *testing.T) {
	ops := []swiftOp{
		{operationID: "rooms.create", method: "POST", path: "/rooms/create",
			input: &swiftSchema{kind: "object", props: []swiftProp{{name: "name", schema: swiftSchema{kind: "string"}, required: true}}},
			errors: []swiftErrorDef{
				{name: "roomLocked", code: "room_locked", status: 409, description: "Room is locked",
					data: &swiftSchema{kind: "object", props: []swiftProp{{name: "locked_until", schema: swiftSchema{kind: "string"}, required: true}}}},
				{name: "room_locked", code: "room_locked_legacy", status: 423, description: "Legacy lock",
					data: &swiftSchema{kind: "object", props: []swiftProp{{name: "retry_after", schema: swiftSchema{kind: "string"}, required: true}}}},
			}},
	}
	out := emitTypeScript(ops, tsGeneratedConfig{URL: "https://x.example.com", APIKey: "k", Branch: "main"})

	// Exactly ONE class for the colliding name (TS2300 guard).
	if n := strings.Count(out, "export class RoomsCreateRoomLockedError"); n != 1 {
		t.Errorf("expected exactly 1 RoomsCreateRoomLockedError class, got %d:\n%s", n, out)
	}
	// Loud comment at the descriptor site.
	must(t, out, `// codegen: skipped error "room_locked" (class name collides with "roomLocked")`)
	// Errors map: only the survivor's code lifts; the dropped code must NOT
	// lift into the survivor's class.
	must(t, out, "errors: { room_locked: (e) => new RoomsCreateRoomLockedError(e) },")
	if strings.Contains(out, "room_locked_legacy:") {
		t.Errorf("dropped error's code must not appear in the errors map:\n%s", out)
	}
	// Orphan Data interface guard: exactly one Data interface (the survivor's),
	// the dropped error's data prop absent.
	if n := strings.Count(out, "export interface RoomsCreateRoomLockedData"); n != 1 {
		t.Errorf("expected exactly 1 RoomsCreateRoomLockedData interface, got %d:\n%s", n, out)
	}
	must(t, out, "locked_until: string;")
	if strings.Contains(out, "retry_after") {
		t.Errorf("dropped error must not emit its Data interface:\n%s", out)
	}
}

// TestEmitTypeScript_ReservedNamespaceDedupCaseInsensitive locks that the
// reserved-namespace skip-comment dedup is case-insensitive (swiftemit
// parity): auth.a + Auth.b produce ONE comment (first-seen casing printed).
func TestEmitTypeScript_ReservedNamespaceDedupCaseInsensitive(t *testing.T) {
	ops := []swiftOp{
		{operationID: "auth.a", method: "POST", path: "/auth/a"},
		{operationID: "Auth.b", method: "POST", path: "/Auth/b"},
	}
	out := emitTypeScript(ops, tsGeneratedConfig{URL: "https://x.example.com", APIKey: "k", Branch: "main"})

	if n := strings.Count(out, "// codegen: skipped reserved namespace"); n != 1 {
		t.Errorf("expected exactly 1 reserved-namespace skip comment, got %d:\n%s", n, out)
	}
	must(t, out, `// codegen: skipped reserved namespace "auth"`)
}
