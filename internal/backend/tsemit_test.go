...
// header + palbe/internal import + configure + single-line empty registration.
// NO `import type ... from 'palbe'` line (neither BackendError nor CallOptions
...
func TestEmitTypeScriptTypedErrors_ZeroOps(t *testing.T) {
	...
	if strings.Contains(out, "import type") {
		t.Errorf("zero ops → no type-only import from 'palbe':\n%s", out)
	}
