...
func emitSwift(ops []swiftOp) string {
	...
	// Generated code calls the SDK's `_invoke` seam, which is
	// `@_spi(Generated)` on Palbe so it stays off `import Palbe`
	...
	// (AnyCodableValue, BackendError, GeneratedFailure, TransportFailure)
	// are plain `public` — Swift 6 rejects SPI types in public API positions.
...
func emitTypeTree(ops []swiftOp) string {
	...
	for i := range ops {
		...
		// EVERY op gets a GeneratedFailure error enum — even with zero
		// inferred errors — so the wrapper's `throws(<Op>Error)` always
		// has a concrete type to name.
		if emitted {
			lines = append(lines, "")
		}
...
// GeneratedFailure` block per operation — EVERY operation gets one, even
// with no inferred errors, so the wrapper's typed-throws signature always
// names a concrete enum. `enumName` is the full top-level name
// (e.g. "RoomsCreateError") so customers reference it directly.
//
...
//
// `init(_ backend:)` is the GeneratedFailure requirement (non-failable):
...
func topLevelErrorEnumLines(enumName string, errs []swiftErrorDef) []string {
	...
	// reason as structLines — GeneratedFailure requires a non-isolated
	// `init(_ backend:)`.
	lines = append(lines, "public nonisolated enum "+enumName+": GeneratedFailure {")
	fd := 1
