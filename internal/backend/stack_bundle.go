...
// arası hiçbir şey onları okumuyor (`bundleOutputDirs`'in kendi yorumu). Bulut
// push'u onları GÖNDERMİYOR bile — `archive.go`: "The cloud builds those
// server-side from source, so the cloud tarball leaves them out". Yani bulut
// kullanıcısının checkout'unda üretilmelerinin hiçbir sebebi yoktu; self-host
// push'u (`BuildStackTarball`) onları paketliyor, ama paketlemek için
// checkout'ta olmaları gerekmiyor — tar onları bu kökten alır.
//
