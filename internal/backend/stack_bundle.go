...
// sweepBuildLitter, ÖLDÜRÜLMÜŞ BİR KOŞUNUN BIRAKTIKLARINI toplar.
//
// `removeBundleOutput` işin SONUNDA koşuyor ve yalnız `defer` ile — SIGINT,
// SIGKILL, panik ya da elektrik kesintisinde o defer KOŞMAZ. İgnore listesinin
// `.palbase-build-controllers` gerekçesi bunu zaten yazıyor: "removed on exit,
// left behind by a SIGKILL".
//
// Aynı sınıf `link`'in hazırlık alanında canlıda ödendi (07.09.2026, müşteri
// deposu): bir gün öncesinden kalmış bir kalıntı, içinde bir API anahtarıyla.
// Sinyal yakalamak çare değil — SIGKILL yakalanamaz. Çare, her koşunun BAŞINDA
// eskiyi toplamasıdır: bu ürünlerin hiçbiri komutlar arası okunmuyor
// (`bundleOutputDirs`'in kendi yorumu), dolayısıyla önceden duran her şey ölüdür.
//
// `.palbase`'İN KENDİSİNE DOKUNULMAZ: `project.json` ve `openapi/` bir ürün
// değil, klonun derleyip push'layabilmesi için gereken SÖZLEŞME. Süpürücü
// yalnız üretilen alt dizinleri alır.
func sweepBuildLitter(dir string) {
	removeBundleOutput(dir)
	for _, legacy := range []string{stagedControllersDir, deployStagingDir} {
		_ = os.RemoveAll(filepath.Join(dir, legacy))
	}
}
...
// bundleRoot, DERLEME ÜRÜNLERİNİN GİTTİĞİ YER — ve müşterinin projesi DEĞİL.
//
// Ürünler (`esm`, `jobs`, `hooks`) tek bir komut boyunca yaşıyor ve komutlar
// arası hiçbir şey onları okumuyor (`bundleOutputDirs`'in kendi yorumu). `push`
// onları tarball'a koyar — bulut ve self-host AYRIMSIZ, ikisi de
// `BuildStackTarball`'dan geçer — ama paketlenmeleri için müşterinin
// checkout'unda DOĞMALARI gerekmiyor: tar onları bu kökten alır.
//
// (İlk hâlinde burada "bulut push'u onları göndermiyor" yazıyordu. YANLIŞTI:
// o cümle `BuildTarball`'ı tarif ediyor ve `push` onu kullanmıyor —
// `stageDeployTree` kullanıyor, deploy'un gördüğünü yerelde taklit etmek için.
// Kullanıcı yakaladı; ölçüm `stack_push.go:297`.)
//
// KÖKÜ ÇAĞIRAN AÇAR: temizliğin sahibi, açan taraf olmalı. Derleyicinin içinde
// bir `defer` bırakmak, aynı `defer`in SIGKILL'de koşmadığı hatanın tekrarı
// olurdu.
func buildStackArtifact(ctx context.Context, dir, bundleRoot string, w io.Writer) ([]uploadUse, *int, error) {
	// ÖNCE ESKİYİ TOPLA. Öldürülmüş bir koşunun bıraktığı bundle, bu koşunun
	// ürettiğiyle karışırsa "yarım bir derlemenin artığını gönderen bir push"
	// mümkün olur — push yolunun kendi yorumunun tarif ettiği hata.
	...
	esmDir := filepath.Join(bundleRoot, ".palbase", "esm", "controllers")
	...
	defer func() {
		if !built {
			...
			removeBundleOutput(bundleRoot)
		}
	}()

	if err := os.WriteFile(filepath.Join(bundleRoot, ".palbase", "esm", "package.json"),
		[]byte(`{"type":"module"}`+"\n"), 0o644); err != nil {
		return nil, nil, err
	}
	...
	if err := writeDefinitionManifests(ctx, bundleRoot, surfaces, w); err != nil {
		return nil, nil, err
	}
