...
func buildStackArtifact(ctx context.Context, dir string, w io.Writer) ([]uploadUse, error) {
	...
	if _, err := os.Stat(controllersDir); err != nil {
		return nil, fmt.Errorf("%s does not exist — a backend is controllers/*.controller.ts "+
	...
	if len(sources) == 0 {
		return nil, fmt.Errorf("no *.module.ts under %s — nothing would answer. A module is "+
	...
	if len(stagedSources) == 0 {
		return nil, fmt.Errorf("staging dropped every module file — the bundle would declare nothing")
	}
