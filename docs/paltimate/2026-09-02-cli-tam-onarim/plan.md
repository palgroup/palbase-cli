...
- [x] **Adım 3: Uzun derlemeleri `-short` dışına al** — `internal/backend/build_test.go` içinde gerçek `bun build`/tsc koşan ve tek başına ≥10 sn süren testlerin başına `if testing.Short() { t.Skip("real bundler run — excluded from the -short budget") }` ekle. ~~Fixture-izole olanlara `t.Parallel()` ekle~~ — **ÇÜRÜTÜLDÜ (deviations D-05):** `seedBackendDir` `t.Chdir` çağırıyor ve `go doc testing.T.Chdir` paralel testte kullanımını yasaklıyor; eklenseydi panik olurdu. Bütçe kapılarla tutuyor.
